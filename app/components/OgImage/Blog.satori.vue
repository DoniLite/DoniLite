...
// Named distinctly from the `title`/`description` props themselves — reusing
// those names here shadowed the props in the template (this component
// always rendered its own default values, e.g. the literal word "title",
// regardless of what was actually passed in, until the caller was fixed).
const displayTitle = computed(() => (props.title || '').slice(0, 60))
const displayDescription = computed(() => (props.description || '').slice(0, 200))
</script>

<template>
  <div class="flex h-full w-full flex-col justify-center bg-[#0d0b07]">
    <svg
